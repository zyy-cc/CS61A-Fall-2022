...
def add_trees(t1, t2):
    ...
    l1, l2 = len(branch1),len(branch2)
    # add both terms
    for i in range(min(l1,l2)):
        merge_branch += [add_trees(branch1[i],branch2[i])]

    # add additional terms
    for j in range(min(l1,l2),max(l1,l2)):
        if l2 > l1:
            merge_branch += [branch2[j]]
        else:
            merge_branch += [branch1[j]]
    return tree(label(t1) + label(t2),merge_branch)
