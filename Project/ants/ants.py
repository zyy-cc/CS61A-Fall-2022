...
class Ant(Insect):
    ...
    def double(self):
        ...
        if self.is_container and self.ant_contained is not None and not self.ant_contained.is_doubled:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            bees = place.bees
            if len(bees) > 0 and (self.lower_bound <= transition <= self.upper_bound):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        #
        #
        # target.action =slow_function(target)
